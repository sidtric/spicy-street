...
export default function LiveOrders() {
  ...
  const fetchOrders = () =>
    axios.get(`${API}/api/orders${filter !== 'all' ? `?status=${filter}` : ''}`, { headers: authHeader() })
  ...
  useEffect(() => {
    const handleNew = (order) => {
      setOrders((prev) => (filter === 'all' || filter === order.status) ? [order, ...prev] : prev);
    };
    ...
    return () => { socket.off('new_order', handleNew); socket.off('order_updated', handleUpdated); };
  }, [filter]);

  const updateStatus = async (id, status) => {
    await axios.patch(`${API}/api/orders/${id}/status`, { status }, { headers: authHeader() });
  };
  ...
  return (
    <div>
      ...
      {orders.length === 0 && <p style={{ color: '#bbb', fontSize: 14 }}>No orders found.</p>}
